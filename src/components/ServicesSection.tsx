      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
          {SERVICES.map((service) => (
            <div
              key={service.id}
              className="bg-white rounded-2xl border border-[#E5DFD7] shadow-sm hover:shadow-md transition-all duration-200 p-6 sm:p-8 flex flex-col justify-between relative group h-full"
            >
              {/* Top Accent Strip */}
              <div className="absolute top-0 left-6 right-6 h-1 bg-[#D95A2B]/20 group-hover:bg-[#D95A2B] transition-colors rounded-t" />

              <div className="flex-1 flex flex-col">
                {/* Header: Icon & Category Tag */}
                <div className="flex items-center justify-between mt-2">
                  <div className="w-12 h-12 rounded-xl bg-[#D95A2B]/10 flex items-center justify-center shrink-0">
                    {getIcon(service.icon)}
                  </div>
                  {service.tag && (
                    <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-[#1C1F22] text-white">
                      {service.tag}
                    </span>
                  )}
                </div>
                ...
                {/* Title */}
                <h3 className="text-xl sm:text-2xl font-bold text-[#1C1F22] mt-5 tracking-tight min-h-[3.25rem] flex items-center">
                  {service.title}
                </h3>
                ...
                <div className="mt-4 p-3.5 rounded-xl bg-[#F8F6F2] border border-[#EBE6DF]">
                  <span className="text-xs font-bold uppercase tracking-wider text-[#D95A2B] block mb-1">
                ...
                {/* Bullet Points */}
                <ul className="mt-5 space-y-2.5 text-xs sm:text-sm text-gray-700 flex-1">
              ...
              {/* Action Link */}
              <div className="mt-6 pt-5 border-t border-[#E5DFD7] flex items-center justify-between shrink-0">
                <a
                  href="#contact"
                  onClick={() => onSelectService && onSelectService(service.title)}
                  className="inline-flex items-center gap-2 text-sm font-bold text-[#D95A2B] hover:text-[#BF491E] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D95A2B] rounded py-1"
                >
