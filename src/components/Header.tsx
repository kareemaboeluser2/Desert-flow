    ...
    <>
      ...
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
          ...
          <a
            href="#top"
            className="flex items-center gap-3 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D95A2B] rounded-lg shrink-0"
            aria-label="DesertFlow Plumbing Home"
          >
            <div className="w-10 h-10 rounded-xl bg-[#D95A2B] flex items-center justify-center text-white shadow-sm shadow-[#D95A2B]/20 transition-transform duration-200 group-hover:scale-105 shrink-0">
              <Droplets className="w-5 h-5 text-white" aria-hidden="true" />
            </div>
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                <span className="font-extrabold text-lg sm:text-xl tracking-tight text-white leading-none whitespace-nowrap">
                  DesertFlow
                </span>
                <span className="text-[10px] uppercase tracking-wider font-bold text-[#D95A2B] bg-[#D95A2B]/15 px-2 py-0.5 rounded border border-[#D95A2B]/30 whitespace-nowrap hidden sm:inline-block">
                  Phoenix, AZ
                </span>
              </div>
              <p className="text-xs text-[#9CA3AF] tracking-normal font-medium mt-1 whitespace-nowrap">
          ...
          {/* Right Action: Desktop Call CTA & Mobile/Tablet Menu Button */}
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            {/* Direct Call Button */}
            <a
              href={`tel:${BUSINESS_INFO.phoneClean}`}
              className="flex items-center gap-2.5 sm:gap-3 bg-[#D95A2B] hover:bg-[#BF491E] active:bg-[#A33D17] text-white px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl font-semibold text-sm transition-all duration-150 shadow-md shadow-[#D95A2B]/20 whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[#1C1F22]"
            ...
            <button
              ...
              className="lg:hidden p-2 text-gray-300 hover:text-white hover:bg-[#282C30] rounded-xl border border-gray-700/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D95A2B] transition-colors"
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-[#1C1F22] border-t border-[#282C30] px-4 pt-4 pb-6 space-y-3 animate-in fade-in slide-in-from-top-2 duration-150 shadow-2xl">
            <nav className="flex flex-col space-y-1" aria-label="Mobile Navigation">
              {navLinks.map((link) => (
                <a
                  ...
                  className="px-3.5 py-2.5 text-base font-semibold text-gray-200 hover:bg-[#282C30] hover:text-white rounded-xl transition-colors"
                >
                  {link.label}
                </a>
              ))}
